...
export function createVpnResources(
  ...
  if (awsToGoogle) {
    ...
    if (isSingleTunnel) {
      // Single
      const externalIp = resources.googleVpnGateways.externalIp.address;
      awsHaVpnGatewayIpAddresses.push(externalIp);
    } else {
      // HA
      if (isComputeHaVpnGateway(resources.googleVpnGateways.vpnGateway)) {
    ...
    if (awsToAzure) {
      ...
      if (isSingleTunnel) {
        const awsAzureVpnConnectionRoutes: RouteConfig[] = [
          {
            target: "Azure",
            cidrBlock: azureVnetResources.vnet.addressSpace[0],
          },
        ];
