...
export function createAwsCustomerGateway(scope: Construct, provider: AwsProvider, params: CustomerGatewayParams) {
  ...
  const logGroup = new CloudwatchLogGroup(scope, `${params.customerGatewayName}-log-group`, {
    provider: provider,
  ...
  const awscGwVpncons = params.awsHaVpnGatewayIpAddresses.map((ipAddress, index) => {
    ...
    if (params.conneectDestination === 'google') {
      vpncon = new VpnConnection(scope, `aws_${params.conneectDestination}_vpn_connection_${index}`, {
        ...commonVpnOptions,
      });
    } else if (params.conneectDestination === 'azure') {
      if (!params.azureVpnProps) {
        throw new Error('Azure VPN properties are required when connecting to Azure');
      }
      vpncon = new VpnConnection(scope, `aws_${params.conneectDestination}_vpn_connection_${index}`, {
        ...commonVpnOptions,
